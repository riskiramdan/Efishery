...
func (a *UserController) PostVerifyToken(w http.ResponseWriter, r *http.Request) {
	...
	var params user.VerifyParams
	errDecode := decoder.Decode(&params)
	if errDecode != nil {
		err = &types.Error{
			Path:    ".UserController->VerifyToken()",
			Message: errDecode.Error(),
			Error:   errDecode,
			Type:    "golang-error",
		}
	...
	var resp interface{}
	errTransaction := a.dataManager.RunInTransaction(r.Context(), func(ctx context.Context) error {
		resp, err = a.userService.VerifyTokenJWT(ctx, params.Token)
		if err != nil {
			return err.Error
		}
		return nil
	...
	if errTransaction != nil {
		err.Path = ".UserController->VerifyToken()" + err.Path
		response.Error(w, "Internal Server Error", http.StatusInternalServerError, *err)
		return
	}
	response.JSON(w, http.StatusOK, resp)
